...
def listar_usuarios():
    ...
    cur = mysql.connection.cursor()
    cur.execute(
        "SELECT id_usuario, nombre, email, rol, telefono, activo FROM usuarios ORDER BY nombre")
...
def agendar_cita():
    try:
        ...
        """, (
            cliente_id,
            animal_id,
            1,  # ID de servicio genérico (podrías crear uno)
            fecha_completa,
            1,  # Estado "Pendiente"
...
def ventas():
    ...
    # Obtener productos disponibles usando el procedimiento almacenado
    cur.callproc('obtener_productos_venta')
    productos = cur.fetchall()

    cur.close()
    return render_template('ventas_simple.html', productos=productos)
...
def procesar_venta():
    try:
        ...
        for producto in productos:
            cur.execute("SELECT precio, stock FROM productos WHERE id_producto = %s", [
                        producto['id']])
            precio, stock = cur.fetchone()

            if producto['cantidad'] > stock:
                flash(
                    f"No hay suficiente stock para el producto ID {producto['id']}", 'danger')
                return redirect(url_for('ventas'))
        ...
        cur.execute(
            "INSERT INTO ventas (fecha, total) VALUES (NOW(), %s)", [total])
        venta_id = cur.lastrowid

        # Registrar detalles de venta y actualizar stock
        for producto in productos:
            cur.execute("SELECT precio FROM productos WHERE id_producto = %s", [
                        producto['id']])
            precio = cur.fetchone()[0]

            # Usar el procedimiento almacenado para agregar detalle
            cur.callproc('agregar_detalle_venta', [
                venta_id,
                producto['id'],
                producto['cantidad'],
                precio
        ...
        flash(
            f'Venta registrada correctamente. Total: ${total:.2f}', 'success')
        return redirect(url_for('ventas'))
...
def pedidos():
    ...
    # Obtener productos con stock bajo usando el procedimiento almacenado
    cur.callproc('productos_para_pedido')
    productos = cur.fetchall()

    # Obtener proveedores
    cur.callproc('obtener_proveedores')
    proveedores = cur.fetchall()

    # Obtener pedidos pendientes
    cur.callproc('obtener_pedidos_pendientes')
...
def procesar_pedido():
    try:
        ...
        # Crear pedido usando el procedimiento almacenado
        cur.callproc('crear_pedido', [proveedor_id, total])
        ...
        for producto in productos:
            ...
            # Usar el procedimiento almacenado para agregar detalle
            cur.callproc('agregar_detalle_pedido', [
                pedido_id,
                producto['id'],
                producto['cantidad'],
                precio
        ...
        flash(
            f'Pedido #{pedido_id} registrado correctamente. Total: ${total:.2f}', 'success')
        return redirect(url_for('pedidos'))
...
def recibir_pedido(pedido_id):
    try:
        ...
        # Usar el procedimiento almacenado para procesar el pedido recibido
        cur.callproc('procesar_pedido_recibido', [pedido_id])
        mysql.connection.commit()
        cur.close()
...
def eliminar_producto():
    try:
        ...
        cur.execute(
            "SELECT id_producto FROM productos WHERE id_producto = %s", [producto_id])
        if not cur.fetchone():
